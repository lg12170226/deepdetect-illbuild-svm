...
public:
  ...
  { 
    ...
    std::string sPathModel= "/home/yangmiao/test/deepdetect-svm/src/svm/model/att_2_10_20170904_sec";
    //dd::CSVM::CSVM();
    int nNum = 10;
    vector<double> vdConf[nNum];
    if (svm.load_model(sPathModel) < 0)
    {
        return;
    }
    ...
    {   
        ...
        for (Value::ConstMemberIterator itr = num.MemberBegin();itr != num.MemberEnd(); ++itr)
        {
            for (int i = 1;i < 11;i++)
            {
                ...
                {
                    vdConf[i-1].push_back(atof(itr->value.GetString()));
                }
            }
        } 
        
        double dConf = svm.predict(vdConf, nNum);
        output_obj.AddMember("name",uridata,allocator);
        output_obj.AddMember("predict", dConf, allocator);
